...
if( !function_exists( 'mars_after_setup_theme' ) ){
	function mars_after_setup_theme() {
		...
		//------------------------------ And Theme Support -----------------------------------------//
		//------------------------------ Add Image Size -----------------------------------------//
		add_image_size('video-featured', 370, 555, true);
		add_image_size('video-lastest', 230, 345, true);
		add_image_size('video-category-featured', 295, 197, true);
		add_image_size('video-item-category-featured', 750, 440, true);
		### sidebar
		add_image_size('most-video-2col', 230, 345, true);
		### Blog
...
if( !function_exists('mars_register_sidebars') ){
	function mars_register_sidebars() {
		...
		register_sidebar( $args = array(
			...
			'before_widget' => '<div class="col-sm-3 widget %2$s">',
			'after_widget'  => '</div>',
		...
		register_sidebar( $args = array(
				'name'          => __( 'Footer Sidebar Info', 'mars' ),
				'id'            => 'mars-footer-sidebar-info',
				'description'   => __('INFO HATA PHIM','mars'),
				'before_widget' => '<div class="col-sm-6 widget %2$s">',
				'after_widget'  => '</div>',
				'before_title'  => '<span class="footer-widget-title">',
				'after_title'   => '</span>'
		)
