					            ...
					            <form name="registerform" id="registerform" action="'.esc_url( site_url('wp-login.php?action=register', 'login_post') ).'" method="post">
					            	...
					            	<p>
						                ...
						                if( isset( $_GET['redirect_to'] ) ){
						                	$content .= '<input type="hidden" name="redirect_to" value="'.esc_url( $_GET['redirect_to'] ).'">';
						                }	                
						              $content .= '
					            </form>						
							';
		...
		function vt_ajax_login(){
			...
			if ( is_wp_error($user) ){
				...
				echo json_encode( array(
					...
					'redirect_to'	=>	!empty( $redirect_to ) ? esc_url( $redirect_to ) : apply_filters('vt_logged_redirect_to', null)
				) );
				exit;
			}
		}
