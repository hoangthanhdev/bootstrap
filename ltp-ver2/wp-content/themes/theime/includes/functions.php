...
if( !function_exists('mars_socials_url') ){
	function mars_socials_url() {
		return array(
			...
			'youtube'	=>	__('Youtube','mars'),
			'vimeo-square'	=>	__('Vimeo','mars')
		);
	}
}
	...
	function mars_orderblock_videos() {
		...
		$block = '<div class="section-nav"><ul class="sorting"><li class="sort-text">'.__('Sort by:','mars').'</li>';
			foreach ( $sort_array as $key=>$value ){
				...
				if( is_search() ){
					$block .= '<li '.$active.'><a href="'. esc_url( add_query_arg( array( 's'=> get_query_var( 's' ), 'order_post' => $key ) ) ) .'">'.$value.'</a></li>';
				}
				else{
...
if( !function_exists('mars_get_socials_count') ){
	function mars_get_socials_count($key) {
		...
		switch ($key) {
			...
			break;
			case 'soundcloud':
				if( function_exists('get_scp_soundcloud') ){
					$count = get_scp_soundcloud();
				}				
			break;
			case 'youtube':
				if( function_exists('get_scp_youtube') ){
					$count = get_scp_youtube();
				}				
			break;
			case 'subscriber':
				$result = count_users();
...
if( !function_exists( 'mars_check_file_allowed' ) ){
	function mars_check_file_allowed( $file, $type = 'video' ){
		...
		if( in_array( $filetype['ext'] , $mimes) ){
			$bool = true;
		}
...
if ( ! function_exists( 'mars_pagination' ) ){
	...
	function mars_pagination( $query = null ) {
		...
		$links = paginate_links( array(
			...
			'prev_next'    => True,
			'prev_text' => !is_rtl() ? __( '&larr; Trang Trước ', 'mars' ) : __( ' &rarr; Trang Trước', 'mars' ),
			'next_text' => !is_rtl() ? __( 'Trang Tiếp &rarr;', 'mars' ) : __( 'Trang Tiếp &larr;', 'mars' )
		) );
...
if( !function_exists( 'mars_hide_js_composer_noted' ) ){
	function mars_hide_js_composer_noted() {
		if( !function_exists( 'vc_set_as_theme' ) )
			return;
		vc_set_as_theme(true);
	}
	//add_action( 'init', 'mars_hide_js_composer_noted' );
}
