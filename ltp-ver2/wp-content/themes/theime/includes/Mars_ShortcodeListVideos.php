		...
		function videotube( $attr, $content ) {
			...
			if( $wpquery->have_posts() ):
				if( $type == 'widget' && $carousel == 'on' ):	
					...
					// default
					?>
					<div class="row columns-<?php print $columns;?> video-section meta-maxwidth <?php print $el_class;?>">
						<?php while ( $wpquery->have_posts() ): $wpquery->the_post(); ?>
							...
							<?php 
							if( $post_type == 'video' ):
								?><span class="title-loop-videos"><a href="<?php print get_permalink( get_the_ID() );?>"><?php print get_the_title( get_the_ID() );?></a></span><?php 
								;
							endif;
							if( $post_type == 'post' ):
								?>
				...
				?><div class="alert alert-info"><?php _e('Mục này hiện chưa được cập nhật phim. Bạn vui lòng quay lại sau hoặc quay lại trang chủ tại link => <a href="http://www.hataphim.com" title="www.hataphim.com">www.hataphim.com</a>','mars');?></div><?php 
			endif;
			wp_reset_postdata();wp_reset_query();
			return ob_get_clean();
		}
